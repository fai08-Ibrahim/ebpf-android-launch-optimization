...
constexpr int WARMUP_CAPTURE_DELAY_MS = 1200;  // allow startup to open files
constexpr int WARMUP_MAX_FILES = 200;
constexpr int WARMUP_READ_BYTES = 64 * 1024;
...
static bool is_valid_warmup_path(const std::string& path) {
    if (path.empty() || path[0] != '/') {
        return false;
    }
    if (path.find("/proc/") == 0 || path.find("/sys/") == 0 || path.find("/dev/") == 0) {
        return false;
    }
    if (path.find("socket:") != std::string::npos || path.find("pipe:") != std::string::npos ||
        path.find("anon_inode:") != std::string::npos || path.find("memfd:") != std::string::npos) {
        return false;
    }
    return true;
}

static bool collect_open_files_from_pid(int pid, std::vector<std::string>& out_paths, bool verbose) {
    std::string cmd = "ls -l /proc/" + std::to_string(pid) + "/fd";
    CommandResult res = execute_command(cmd, true);
    if (!res.success) {
        if (verbose) {
            std::cerr << "Warm-up: failed to list /proc/" << pid << "/fd" << std::endl;
        }
        return false;
    }
    ...
    while (std::getline(iss, line)) {
        ...
        std::string path = trim_copy(line.substr(arrow + 3));
        size_t deleted_pos = path.find(" (deleted)");
        if (deleted_pos != std::string::npos) {
            path = trim_copy(path.substr(0, deleted_pos));
        }
        if (!is_valid_warmup_path(path)) {
            continue;
        }
        if (seen.find(path) == seen.end()) {
            seen[path] = true;
            out_paths.push_back(path);
            if ((int)out_paths.size() >= WARMUP_MAX_FILES) {
                break;
            }
        }
    }
    ...
    if (verbose) {
        std::cout << "[WARMUP] Captured " << out_paths.size() << " file paths" << std::endl;
    }
    return !out_paths.empty();
}

static bool write_warmup_list(const std::string& path, const std::vector<std::string>& files, bool verbose) {
    std::ofstream ofs(path);
    if (!ofs.is_open()) {
        std::cerr << "Warm-up: failed to write " << path << std::endl;
        return false;
    }
    ...
    ofs.close();
    if (verbose) {
        std::cout << "[WARMUP] Wrote warm-up list to " << path << std::endl;
    }
    return true;
}

static bool warmup_read_files(const std::string& path, bool verbose) {
    std::ifstream ifs(path);
    if (!ifs.is_open()) {
        std::cerr << "Warm-up: failed to open list " << path << std::endl;
        return false;
    }

    std::vector<char> buffer(WARMUP_READ_BYTES);
    std::string file_path;
    int warmed = 0;

    while (std::getline(ifs, file_path)) {
        file_path = trim_copy(file_path);
        if (!is_valid_warmup_path(file_path)) {
            continue;
        }
    ...
    if (verbose) {
        std::cout << "[WARMUP] Touched " << warmed << " files" << std::endl;
    }
    return warmed > 0;
}
...
bool dump_run_summary(const std::string& output_path, const RunResult& result,
    ...
    ofs << result.run_number << ","
        << (cfg.enable_warmup ? "warmup_files" : "baseline") << ","
        << cfg.package_name << ","
...
static bool ensure_warmup_list(const ExperimentConfig& cfg) {
    std::string list_path = cfg.output_dir + "/warmup_files.txt";
    if (file_exists(list_path)) {
        return true;
    }

    if (cfg.verbose) {
        std::cout << "[WARMUP] Creating warm-up list at " << list_path << std::endl;
    }

    force_stop_app(cfg.package_name, cfg.verbose);
    return_to_home();

    std::string component = cfg.package_name + "/" + cfg.activity_name;
    ...
    usleep(WARMUP_CAPTURE_DELAY_MS * 1000);

    int pid = get_process_pid(cfg.package_name);
    if (pid < 0) {
        std::cerr << "Warm-up: failed to find PID for " << cfg.package_name << std::endl;
        force_stop_app(cfg.package_name, cfg.verbose);
        return false;
    }

    std::vector<std::string> paths;
    bool ok = collect_open_files_from_pid(pid, paths, cfg.verbose);
    force_stop_app(cfg.package_name, cfg.verbose);
    if (!ok) {
        return false;
    }

    return write_warmup_list(list_path, paths, cfg.verbose);
}

static bool perform_file_warmup(const ExperimentConfig& cfg) {
    if (!cfg.enable_warmup) {
        return true;
    }

    std::string list_path = cfg.output_dir + "/warmup_files.txt";
    if (!ensure_warmup_list(cfg)) {
        return false;
    }

    if (cfg.verbose) {
        std::cout << "[WARMUP] Warming files from " << list_path << std::endl;
    }
    return warmup_read_files(list_path, cfg.verbose);
}
...
RunResult run_cold_launch_once(const ExperimentConfig& cfg,
    ...
    // Step 3: Warm page cache using file list
    if (cfg.verbose) {
        std::cout << "[3/8] Warm-up phase" << std::endl;
    }
    if (!perform_file_warmup(cfg)) {
        result.error_msg = "Warm-up failed";
        return result;
    }
    
    // Step 4: Clear BPF maps from previous run
    if (cfg.verbose) {
        std::cout << "[4/8] Clearing BPF aggregation maps" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    const char* warmup_env = std::getenv("EBPF_WARMUP");
    if (warmup_env && std::strcmp(warmup_env, "1") == 0) {
    ...
    std::cout << "Mode: " << (cfg.enable_warmup ? "warmup_files" : "baseline") << std::endl;
    std::cout << std::endl;
