...
static bool g_kernel_opt_enabled = false;
...
constexpr const char* PINNED_LAUNCH_TGID_MAP = "/sys/fs/bpf/map_example_launch_tgid_map";
...
bool reset_launch_tgid(int launch_tgid_fd, bool verbose) {
    if (launch_tgid_fd < 0) return false;
    int key = 0;
    uint32_t zero = 0;
    bpf_map_update_elem(launch_tgid_fd, &key, &zero, BPF_ANY);
    if (verbose) {
        std::cout << "[BPF] Reset launch TGID" << std::endl;
    }
    return true;
}
bool return_to_home() {
...
LaunchResult launch_app_and_measure_boosted(const std::string& package,
                                            const std::string& activity,
                                            int timeout_ms,
                                            int launch_tgid_fd,
                                            bool verbose) {
    ...
    bool boosted = false;
    uint32_t launch_tgid = 0;
    for (int i = 0; i < 50; i++) {
        if (launch_tgid_fd >= 0) {
            int key = 0;
            if (bpf_map_lookup_elem(launch_tgid_fd, &key, &launch_tgid) == 0 && launch_tgid != 0) {
                std::string renice_cmd = "renice -n -10 -p " + std::to_string(launch_tgid);
                execute_command(renice_cmd, false);
                boosted = true;
                break;
            }
        }
    ...
    if (!boosted && verbose) {
        std::cout << "[KOPT] Launch TGID not captured" << std::endl;
    }
    ...
    while (std::getline(iss, line)) {
        if (line.find("TotalTime:") != std::string::npos) {
            ...
            std::string time_str = line.substr(pos + 10);
            errno = 0;
            const char *tstart = time_str.c_str();
            char *tend = nullptr;
            long long parsed = std::strtoll(tstart, &tend, 10);
            if (errno == 0 && tend != tstart) {
                result.total_time_ms = parsed;
                found_total_time = true;
            }
        }
...
bool dump_run_summary(const std::string& output_path, const RunResult& result,
    ...
    const char* mode = g_kernel_opt_enabled ? "kernel_opt" : "baseline";
...
RunResult run_cold_launch_once(const ExperimentConfig& cfg,
    ...
    if (g_kernel_opt_enabled) {
        reset_launch_tgid(launch_tgid_fd, cfg.verbose);
    }
    ...
    LaunchResult launch = g_kernel_opt_enabled
        ? launch_app_and_measure_boosted(cfg.package_name,
                                         cfg.activity_name,
                                         cfg.launch_timeout_ms,
                                         launch_tgid_fd,
                                         cfg.verbose)
        : launch_app_and_measure(cfg.package_name,
                                 cfg.activity_name,
                                 cfg.launch_timeout_ms,
                                 cfg.verbose);
...
struct BPFContext {
    ...
    BPFContext() : file_open_count_fd(-1), order_count_fd(-1),
                   latency_max_fd(-1), launch_tgid_fd(-1) {}
};
...
bool load_and_attach_bpf(BPFContext& ctx, bool verbose) {
    ...
    ctx.launch_tgid_fd = bpf_obj_get(PINNED_LAUNCH_TGID_MAP);
    ...
    if (ctx.launch_tgid_fd < 0) {
        std::cerr << "Error: failed to open " << PINNED_LAUNCH_TGID_MAP
                  << ": " << strerror(errno) << std::endl;
    }
    ...
    if (verbose) {
        ...
        std::cout << "  " << PINNED_LAUNCH_TGID_MAP << " (fd=" << ctx.launch_tgid_fd << ")" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    const char* opt_env = getenv("EBPF_Kernel_optimization");
    if (opt_env && (opt_env[0] == '1' || opt_env[0] == 'y' || opt_env[0] == 'Y' || opt_env[0] == 't' || opt_env[0] == 'T')) {
    ...
    std::cout << "Mode: " << (g_kernel_opt_enabled ? "kernel_opt" : "baseline") << std::endl;
    std::cout << std::endl;
